...
class Server:
    ...
    def get_mean(self):
        ...
        return mean([p.processing_time/self.velocity for p in self.queue])

    def get_std(self):
        if len(self.queue) == 0:
            return 0

        l = [p.processing_time/self.velocity for p in self.queue]
        mean = sum(l) / len(l)
    ...
    def get_percentile(self, percentile=90):
        ...
        a = np.array([p.processing_time/self.velocity for p in self.queue])
        return np.percentile(a, percentile)

    def get_observation(self):
        return len(self.queue), self.get_mean(), self.get_std(), self.get_percentile()
...
class NetworkEnv(gym.Env):
    def __init__(self, num_packets=0, num_servers=0, num_balancers=0, collaborative=True, server_velocity=None, gamma=0.9, num_clients=50):
        ...
        self.packets = [Packet(ip=random.randint(0, num_clients - 1), time_received=random.random(), processing_time=np.random.exponential(0.004)) for i in
                        range(num_packets)]
    ...
    def step(self, action_n):
        ...
        for i in aa:
            ...
            alpha.append(i)
            k = random.random()
            if k > 0.3/(1+1e-4)**self.index:
                a.append(np.random.choice(np.arange(0, len(self.servers)), p=i))
            else:
                lst = np.arange(0, len(self.servers)).tolist()
                j = i.copy()
                index = np.argmax(i)
                del(lst[index])
                del(j[index])
                ssum=sum(j)
                j = [(p / ssum) for p in j]
                a.append(np.random.choice(lst, p=j))
            entropy.append(alpha[count][a[count]])
            count += 1
        action_n = a
        ...
        for server in self.servers:
            l, m, s, n = server.get_observation()
            length.append(l)
            mean.append(m)
            std.append(s)
            percentile.append(n)
        for server in self.servers:
            l, m, s, n = server.get_observation()
            temp.append((l - min(length) + 0.5) / (max(length) - min(length) + 0.5))
            temp.append((m - min(mean) + 0.5) / ((max(mean) - min(mean) + 0.5) * server.velocity))
            temp.append((s - min(std) + 0.5) / (max(std) - min(std) + 0.5))
            temp.append((n - min(percentile) + 0.5) / (max(percentile) - min(percentile) + 0.5))
        obs = temp #[temp] * self.n

        new_loads = [self.get_stochastic_load(packets=packet, j=j, alpha=alpha) for j in range(len(self.servers))]
        if self.t == 0:
            ...
            for i in range(len(self.agents)):
                reward += -0.01*math.log(entropy[i])
            self.loads = new_loads
        self.t += 1
    ...
    def reset(self):
        self.packets = [Packet(ip=random.randint(0, self.num_clients - 1), time_received=random.random(), processing_time=np.random.exponential(0.004)) for i in
                        range(num_packets)]
        self.packets.sort(key=lambda x: x.time_received, reverse=False)
        self.time = 0  # 当前的时刻
        self.index = 0
...
if __name__ == '__main__':
    ...
    for e in range(1, episodes + 1):
        ...
        while not done:
            action_n = th.Tensor([ [0.025]*4+[0.225]*4 for i in range(env.n)])
